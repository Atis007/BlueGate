...
# DUMMY MODE TOGGLE - Set to True to test without real BLE devices or Supabase
DUMMY_MODE = True

if not DUMMY_MODE and (not SUPABASE_URL or not SUPABASE_KEY):
...
def student_exists(student_id: str) -> bool:
    if DUMMY_MODE:
        # Dummy mode: accept these two test students
        return student_id in ["student12345678", "student87654321"]
...
def insert_attendance(student_id: str, rssi: int):
    if DUMMY_MODE:
        print(f"[DUMMY] Mentve: {student_id} | RSSI={rssi} dBm")
        return
...
async def scan_once():
    ...
    print("Mode:", "DUMMY" if DUMMY_MODE else "LIVE")
    ...
    if DUMMY_MODE:
        # Fake two devices for testing
        print("[DUMMY] Generálunk 2 teszt eszközt...\n")
        await asyncio.sleep(2)  # simulate scan delay
        
        class DummyDevice:
            def __init__(self, address, name, student_id, rssi):
                self.address = address
                self.name = name
                self.rssi = rssi
                self._student_id = student_id
                
                # Mock advertisement_data
                class DummyAdData:
                    def __init__(self, sid):
                        self.service_data = {
                            APP_SERVICE_UUID: sid.encode("utf-8")
                        }
                
                self.metadata = {"advertisement_data": DummyAdData(student_id)}
        
        devices = [
            DummyDevice("AA:BB:CC:DD:EE:01", "BlueGate Student 1", "student12345678", -44),
            DummyDevice("AA:BB:CC:DD:EE:02", "BlueGate Student 2", "student87654321", -61),
        ]
    else:
    ...
    for dev in devices:
        ...
        # Mentés Supabase-be
        insert_attendance(student_id, rssi)  # Now active in dummy mode too!
